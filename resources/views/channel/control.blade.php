    ...
    <script type="text/javascript">
        ...
        function sendAdmin() {
            ...
            $.post("/api/channel/sendAdmin", {
                ...
                if (res == "success") {
                    var unimsg=String(new Date().getTime())+uname+";"+$('#adminText').val();
                   ...
                   iziToast.warning({
                    ...
                    var text1='{"msg":[{"uname":"'+uname+'","action":"0","usign":"就佳佳佳佳佳佳佳佳佳佳佳佳","ugood":"17262","roomnum":"","level":"1","timestamp":"00:03","equipment":"app","uid":"1","message":"'+unimsg+'","ct":"'+$('#adminText').val()+'","touid":"17262","_method_":"SendMsg","msgtype":"2","tougood":"","uhead":"'+avatar+'","bullet":"0","touname":""}],"retmsg":"OK","retcode":"000000"}';
                    socket2.emit('broadcast',JSON.parse(text1));
        ...
        /***************************************6006start********************/
        var avatar,uname;
        console.log("第二个socket开始创建");
        var socket2=io("http://appapi.scclbl.com:6006");
        var uid='1';
        var token="c454540b91ea52ed95e92ca3fbc5d8c9";
        $.ajax({
            url: 'http://appapi.scclbl.com/v3/public/?service=User.setNodejsInfo&uid=1&token=c454540b91ea52ed95e92ca3fbc5d8c9&showid=2',
            data: {
            ...
            uname=data.data.info.user_nicename;

                socket2.on('connect',function(){
                    console.log("socket2 is connect");
                    socket2.emit('conn',{uid:uid,token:token,roomnum:channel_id});
                });
            }
        ...
        socket2.on('broadcastingListen',function(data){
                data=JSON.parse(data);
                if(data.msg!=undefined){
                    for(var i=0;i<data.msg.length;i++){
                        switch(data.msg[i]._method_){
//                        console.log(data.msg[i]);
                            ...
                            console.log("received Data: "+data);
                                if(data.msg[i].uid>=1){
                                       ...
                                       var msgInfo=data.msg[i];
                                       var key=msgInfo.message;
                                       console.dir("key ="+key);
                                       var isNeedLoad=true;
                                       if(chatRoom.messages.length){
                                           $.each(chatRoom.messages, function(index, value, array) {
                                               if(value.unimsg&&value.unimsg==key){
                                                    console.log(""+value.unimsg);
                                                    chatRoom.messages.slice(index,1);
                                                    isNeedLoad=false;
                                                    return false;
                                               }
                                           });
                                       ...
                                       if(isNeedLoad){
                                           console.log("msg has not been loaded");
                                           var time1 = new Date().Format("yyyy-MM-dd hh:mm:ss");
                                           msgInfo.name=msgInfo.uname;
                                           msgInfo.message=msgInfo.ct;
                                           msgInfo.created_at=time1;
                                           msgInfo.image=msgInfo.avatar;
                                           chatRoom.messages.unshift(msgInfo);
                                       }

                                 /*   var mege=data.msg[i].ct;
                                    var name=data.msg[i].uname!=undefined?data.msg[i].uname:data.msg[i].user_nicename;
                                   console.log("received mege:"+mege);


                                   var append_html='<li class="coment-content" >'+
                                                                  '<img src="'+avatar+'" width="50px" height="50px" alt="">'+
                                                                  '<span class="content-item-top-right">'+
                                                                          '<h4 style="margin:10px 0 20px 0;">'+name+'</h4>'+
                                                                          '<p style="margin-bottom:0;display:inline-block;float:right;margin-top: 8px;"> <span>'+time1+'</span></p>'+
                                                                          '<p>'+mege+'</p>'+
                                                                      '</span>'+
                                                              '</li>';
                                    $("#chatRoom").prepend(append_html);*/
                                }
                                break;
                            case 'SendGift':
                                break;
        ...
        socket2.on('disconnect',function(){
            console.log("socket2 is disconnect");
//            socket2.socket.reconnect();
        });

        socket2.on('reconnect',function(){
            console.log("socket2 is reconnected");
        });


        socket2.on('reconnecting',function(){
            console.log("socket2 is reconnecting");
        });
