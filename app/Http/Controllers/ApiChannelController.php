...
public function video($channel_id,Request $request){

   
    if(!empty($_FILES['foretell_content_video'])){
        $video=$_FILES['foretell_content_video'];
         $vn=$request->video_name;
    }
    else{
        // return $_FILES['foretell_content_img'];
        $video=$_FILES['foretell_content_img'];
        $vn=$request->img_name;


   $a=$video['tmp_name'];
   $n=$vn;
     $savepath = 'upload/video/'.time(); 
$a = move_uploaded_file($a,$savepath.$n) ;

        $foretell=Foretell::where('channel_id',$channel_id)->get();
        $name=$request->foretell_name;
        $time=$request->foretell_time;
        $info=$request->foretell_info;
       $data['foretell_content_img']=$savepath.$n;
       $data['foretell_content_video']='';
        $data['foretell_name']=$name;
        $data['foretell_time']=$time;
        $data['foretell_info']=$info;



        if($foretell->isEmpty()){
             $data['channel_id']=$channel_id;
            $a=Foretell::create($data);
        ...
        $img=Foretell::select('foretell_content_img','foretell_content_video')->where('channel_id',$channel_id)->get();
        $a= $img[0]['foretell_content_img'];
         $b= $img[0]['foretell_content_video'];
         @unlink($a);
         @unlink($b);
        $b=Foretell::where('channel_id',$channel_id)->update($data);
        if($b){
             $msg['code']=0;
                $msg['data']='success';
                return $msg;
...
$a = move_uploaded_file($a,$savepath.$n) ;
        ...
        $time=$request->foretell_time;
        $info=$request->foretell_info;
       $data['foretell_content_video']=$savepath.$n;
       $data['foretell_content_img']='';
        $data['foretell_name']=$name;
        $data['foretell_time']=$time;
        $data['foretell_info']=$info;
 ...
 $vid=Foretell::select('foretell_content_img','foretell_content_video')->where('channel_id',$channel_id)->get();
        $a= $vid[0]['foretell_content_img'];
         $b= $vid[0]['foretell_content_video'];
         @unlink($a);
         @unlink($b);
        $b=Foretell::where('channel_id',$channel_id)->update($data);
        if($b){
...
// var_dump($res['result']['outputGroupList'][0]['id']);die;
        if (($res['code']== 0 && $screen == 1) || ($res['code']== 0 && $screen == 2 )) {
            ...
            Channel::create([
                ...
                'channel_id' => $res['result']['id'],
                 'play_url' => $res['result']['playUrl'],
                  'push_url' => $res['result']['pushUrl'],
                  // 'live_url' => Config::get('app.url') ."/live/" . $channel_id.'/'.$ak.'/'.$as,
                'logo' =>$savepath ,
      ...
      $guide= Channel::where('id',$channel_id)->update([
       ...
       if($guide){
        ...
        public function qr($channel_id,$ak,$as){
            $qrcode = new BaconQrCodeGenerator;
            $msg['url']= Config::get('app.url') ."/live/" . $channel_id.'/'.$ak.'/'.$as;
 
       
         
            return $msg;
            return $qrcode->size(200)->generate(Config::get('app.url') ."/live/" . $channel_id.'/'.$ak.'/'.$as) ;  
        }
...
public function waysee($channel_id,Request $request){

if($request->type==0){
	
	$tdata=DB::table('channel_type')->where('channel_id',$channel_id)->get();
	 if(!$tdata->isEmpty()){
        $a=DB::table('channel_type')
    ...
    if($request->type==1){
       ...
       // return $a['hint'];die;
        ...
        $data['max_num']=$a['max_num'];
        $data['channel_id']=$channel_id;
        // Channel::where('id',$channel_id)->update($m);
        $tdata=DB::table('channel_type')->where('channel_id',$channel_id)->get();
        if(!$tdata->isEmpty()){
        $a=DB::table('channel_type')
        ->where('channel_id',$channel_id)
...
public function see($channel_id){

   $data= DB::table('channel_type')->where('channel_id',$channel_id)->get();
   if($data->isEmpty()){
...
public function foretell($channel_id){
    $data=Channel::select('id','foretell_open')->where('id',$channel_id)->get();
    $typedata=Type::select('type')->where('channel_id',$channel_id)->get();
 ...
 $typedata=(array)json_decode($typedata)[0];
    }
    $data=(array)json_decode($data)[0];
    
     $foretell['type']=$typedata['type'];
     $foretell['foretell_open']=$data['foretell_open'];
     $foretelldata=Foretell::where('channel_id',$channel_id)->get();
     if( $foretelldata->isEmpty()){
        $foretell['foretell']='';

     }
     else{
        $foretelldata=(array)json_decode($foretelldata)[0];
     }
     
     $foretell['foretell']=$foretelldata;
     $msg['data']=$foretell;
     return $msg;
    
}
public function  foretellclose($channel_id){
    $data['foretell_open']=0;
    $close=Channel::where('id',$channel_id)->update($data);
    if($close){
        $msg['code']=0;
        $msg['data']='success';
...
public function  foretellopen($channel_id){
    $data['foretell_open']=1;
    $open=Channel::where('id',$channel_id)->update($data);
    if($open){
        $msg['code']=0;
        $msg['data']='success';
  ...
  public function usercompany($user_id,Request $request){
    $logo=$_FILES['company_logo'];
     $a=$logo['tmp_name'];
    $n=$logo['name'];
    $logopath = 'upload/video/'.time(); 
    $a = move_uploaded_file($a,$logopath.$n) ;

    $card_a=$_FILES['card_a'];
    $a=$card_a['tmp_name'];
    $n=$card_a['name'];
    $cardapath = 'upload/video/'.time(); 
    $a = move_uploaded_file($a,$logopath.$n) ;

    $card_b=$_FILES['card_b'];
     $a=$card_b['tmp_name'];
    $n=$card_b['name'];
    $cardbpath = 'upload/video/'.time(); 
    $a = move_uploaded_file($a,$logopath.$n) ;
    ...
    $data['content_way']=$request->content_way;
    $data['content_person']=$request->content_person;
    $data['card_a']=$cardapath;
    $data['card_b']=$cardbpath;
    $data['user_id']=$user_id;
    $statu=Usercompany::create($data);
    if($statu){
        $msg['code']=0;
        $msg['data']='success';
        return $msg;
