...
//  $channels = Channel::select('name','user_id','updated_at','open','screen','id','logo','manager_id')->where('company_id', $request->companyid)->where('user_id',$request->userid)->where('name','like','%'.$name.'%')->offset($of)->limit(4)->get();
// }
    if($channels->isEmpty()){
...
public function listsearch(Request $request){
    $name=$request->name;
     // $of=4*($request->page-1);
    $channels = Channel::select('name','user_id','updated_at','open','screen','id','logo','manager_id')->where('company_id', $request->companyid)->where('user_id',$request->userid)->where('name','like','%'.$name.'%')->get();
